...
function ensureModel(name, schema) {
  ...
  if (existing) {
    const hasSchemaDrift = Object.keys(schema.paths).some(
      (pathName) => !existing.schema.path(pathName)
    );
...
const offerSchema = new Schema({
  ...
  pointsCost:    { type: Number, default: 25, min: 1 },
  startTime:     { type: Date },
  endTime:       { type: Date },
...
const redemptionSchema = new Schema({
  ...
  redemptionType: { type: String, enum: ['reward', 'offer'], default: 'reward' },
...
redemptionSchema.index({ offerId: 1 });


// ----- Model Exports -----
module.exports = {
  User:        ensureModel('User', userSchema),
  Restaurant:  ensureModel('Restaurant', restaurantSchema),
  Dish:        ensureModel('Dish', dishSchema),
  Review:      ensureModel('Review', reviewSchema),
  Offer:       ensureModel('Offer', offerSchema),
  Reward:      ensureModel('Reward', rewardSchema),
  Redemption:  ensureModel('Redemption', redemptionSchema),
};
